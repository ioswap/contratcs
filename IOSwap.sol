...
contract UniswapV2ERC20 is IUniswapV2ERC20, Initializable {
    ...
    string public constant name = 'IOSwap Liquidity Provider Token';
    string public constant symbol = 'IOLP';
...
contract IOSwapPair is IUniswapV2Pair, UniswapV2ERC20 {
    using SafeMath  for uint;
    ...
    uint private locked;
    modifier lock() {
        require(locked == 0, 'IOSwap: LOCKED');
        locked = 1;
        _;
        locked = 0;
    }
...
contract IOSwapFactory is IUniswapV2Factory {
    using SafeMath for uint;
    ...
    function initialize(address _feeToSetter, address _productImplementation) public {
        require(feeToSetter== address(0) && _feeToSetter != address(0), 'IOSwapFactory.initialize can be delegatecall once by proxy only.');
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        ...
        require(token0 != address(0), 'IOSwapFactory: ZERO_ADDRESS');
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure returns (uint amountB) {
        require(amountA > 0, 'IOSwapFactory: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'IOSwapFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOutPair(address pair, uint amountIn, uint reserveIn, uint reserveOut) public view returns (uint amountOut) {
        require(amountIn > 0, 'IOSwapFactory: INSUFFICIENT_INPUT_AMOUNT');
    ...
    function getAmountInPair(address pair, uint amountOut, uint reserveIn, uint reserveOut) public view returns (uint amountIn) {
        require(amountOut > 0, 'IOSwapFactory: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'IOSwapFactory: INSUFFICIENT_LIQUIDITY');
        (uint _feeRate, ) = getFeeRate(pair);
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'IOSwapFactory: INVALID_PATH');
...
contract IOSwapRouter02 is IUniswapV2Router01, IUniswapV2Router02, Initializable {
    using SafeMath for uint;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {      // virtual 
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = IOSwapFactory(factory).getAmountOut(amountInput, reserveInput, reserveOutput);
            }
        ...
        returns (uint amountOut)
    {
        ...
        return IOSwapFactory(factory).getAmountOut(amountIn, reserveIn, reserveOut);
    }
        ...
        returns (uint amountIn)
    {
        ...
        return IOSwapFactory(factory).getAmountIn(amountOut, reserveIn, reserveOut);
    }
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        ...
        //        factory,
        //        keccak256(abi.encodePacked(token0, token1)),
		//		IOSwapFactory(factory).pairCodeHash                                    //hex'96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f' // init code hash
        //    ))));
        return IOSwapFactory(factory).getPairFor(tokenA, tokenB);
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        //    amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut);
        //}
        return IOSwapFactory(factory).getAmountsOut(amountIn, path);
    }
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        //    amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut);
        //}
        return IOSwapFactory(factory).getAmountsIn(amountOut, path);
    }
}
...
contract DeployFactory {
    ...
    constructor(address adminFactory, address adminPair) public {
        IOSwapPair pair = new IOSwapPair();
        emit Deploy('IOSwapPair', address(pair));
        
        IOSwapFactory factory = new IOSwapFactory();
        emit Deploy('IOSwapFactory', address(factory));
